...
void modeCPAP_PS(RespSettings_t* Settings, MeasuredParams_t* Measured, CtrlParams_t* Control)
{
...
*/
  ...
  FilteredFlow=FIRflow(Measured->flow,0,0);
	if (dihanje_state >= MODE_STATE_INSP_INIT)
  {
...
#define MIN_FLOW_TO_START_MONITORING  -10
      ...
      if (Settings->new_mode != MODE_CPAP_PS)
      {
...
//P-ramp  ///////////////////////////////////////////////////
  ...
  //    Control->target_pressure = SETPrampStartPressure + (SETPrampPressure-SETPrampStartPressure)/SETpramp_time*timing;
      ...
      timing += TIME_SLICE_MS;
  //    Control->target_pressure = SETPrampStartPressure + (SETPrampPressure-SETPrampStartPressure)/SETpramp_time*timing;
      Control->target_pressure = FIRtargetP(SETPrampStartPressure + (SETPrampPressure-SETPrampStartPressure)/SETpramp_time*timing,0,0);
      if (timing >= SETpramp_time)	// gremo v constant pressure
      {
        Control->target_pressure = FIRtargetP(SETPrampPressure,0,0);
        dihanje_state=MODE_STATE_INSP_CONST_P;
...
//Constant pressure ////////////////////////////////////////
      ...
      timing += TIME_SLICE_MS;
      Control->target_pressure=FIRtargetP(SETPrampPressure+ (timing-SETpramp_time)*PRESSURE_INCREMENT,0,0);
    ...
    case MODE_STATE_INSP_COMPLETE_MAX_VOL: //(Only in case of Error in previous state) motor je prisel do konca, pocakaj, da mine cas vdiha
      ...
      break;
		...
		default:
      ReportError(ModeCPAP_PS_UnknownState,FSH("Error: Unknown state in C_VCV state machine"));
      Control->mode=CTRL_PAR_MODE_STOP;
      dihanje_state = 0;
      break;
	}
